...
try {
  ...
  console.log('Vite version error:', e.message?.slice(0, 500));
}

console.log('\n=== Trying to load vite config ===');
try {
  const result = execSync('node -e "import(\\\"vite\\\").then(v => console.log(\\\"vite loaded OK\\\")).catch(e => console.log(\\\"vite load error:\\\", e.message))" 2>&1', { encoding: 'utf8', timeout: 15000 });
  console.log(result.trim());
} catch(e) {
  console.log('Config load error:', e.message?.slice(0, 500));
}
